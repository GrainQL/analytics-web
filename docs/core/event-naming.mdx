...
The hierarchical naming convention makes backend queries incredibly powerful:

### Filter by Category
```sql
-- All user-related events
SELECT * FROM events WHERE eventName LIKE 'user_%'

-- All checkout events
SELECT * FROM events WHERE eventName LIKE 'checkout_%'
```

### Filter by Action
```sql
-- All click events across your app
SELECT * FROM events WHERE eventName LIKE '%_clicked'

-- All form submissions
SELECT * FROM events WHERE eventName LIKE '%_submitted'
```

### Combine Filters
```sql
-- All cart additions and removals
SELECT * FROM events WHERE eventName IN ('cart_item_added', 'cart_item_removed')

-- All product interactions that are views
SELECT * FROM events WHERE eventName LIKE 'product_%_viewed'
```
...
**Feature adoption**:
```typescript
// Track all interactions with a specific feature
const featureEvents = await queryEvents({
  eventName: 'content_video_*' // All video events
});
```
