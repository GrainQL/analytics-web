...
|------|-------------|----------|
| **Opt-In** | GDPR strict – explicit consent required | EU users, strict compliance |
| **Opt-Out** | Track by default, opt-out available | US users, CCPA compliance |
| **Disabled** | No consent management | Internal tools, non-regulated |

```typescript
const grain = createGrainAnalytics({
  tenantId: 'your-tenant-id',
  consentMode: 'opt-in',  // or 'opt-out', 'disabled'
  waitForConsent: true
});
...
See the [Consent Management Guide](/guides/consent-management) for code examples and best practices.

## Next Steps

<CardGroup cols={2}>
  <Card title="Consent Management" icon="shield-check" href="/guides/consent-management">
